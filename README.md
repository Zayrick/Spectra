...
插件注释 ──VID/PID 触发索引──> HID 扫描 / 热插拔
                                  │ 任一 @hid 命中时启用插件
                                  ▼
设备发现 Lua VM ──discover(hids)──> 已注册的逻辑设备 ──> 按设备隔离的 GUI 页面状态
                                                               │
                                                               ├─设备 A──> 实时管线 A
                                                               ├─设备 B──> 实时管线 B
                                                               └─设备 N──> 单机事务 N

实时管线 ──> 60 Hz 灯效 VM ──> Skia GPU ──RGB bytes──> 设备 Lua VM
单机事务 ──> open ──> apply_mode ──> close
设备 Lua VM / 单机事务 ──> hidapi ──> HID device
```
...
- 在左侧设备列表中选择设备；
- 每台设备分别保存当前页面、灯效选择、单机模式参数和颜色编辑状态；切换设备不会覆盖
  其他设备的页面；
- 在“实时”页面点击灯效即可为当前设备淡入启动，切换到其他灯效时平滑过渡；点击正在
  运行的灯效即可淡出熄灭并停止该设备的管线。多台设备可同时运行不同实时灯效；
- 在“单机”页面选择设备端模式，使用插件为该模式声明的控件调整参数，然后应用；
- 点击标题栏的“扫描设备”立即刷新设备列表；后台也会每秒自动扫描热插拔；
- 关闭窗口时，应用会停止所有活动实时管线并关闭对应设备会话。
...
每台设备拥有独立实时管线和页面状态。启动、停止、故障和热插拔只影响对应设备；各管线
分别创建 effect worker、device worker、Lua VM 和单槽邮箱。单机模式使用独立设备事务，
内核只停止目标设备的实时管线，再在后台打开 HID 会话、应用插件声明的模式参数并关闭
会话。不同设备的单机事务可以并行执行。
...
- 设备插件通过 HID 与设备通信；
- GUI 可让多台设备同时运行彼此独立的实时灯效，也可分别配置设备端单机模式；
- 热插拔每秒刷新一次，拔出设备只会停止该设备的管线；
